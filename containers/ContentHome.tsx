...
//                                 </div>
//                                 <p className="text-base font-semibold text-white">
//                                   {product?.prices && product.prices.SOL ? (
//                                     <>
//                                       {product.prices.SOL.toLocaleString("vi")} SOL
//                                     </>
//                                   ) : null}
//                                   {product?.prices && product.prices.USDC ? (
//                                     <>
//                                       {product.prices.USDC.toLocaleString("vi")} USDC
//                                     </>
//                                   ) : null}
//                                 </p>
//                               </div>
...
const ContentHome = () => {
  ...
  const [products, setProducts] = useState<ListProduct[]>();
  const [totalProduct, setTotalProduct] = useState(0);
  const [usdcToSolRate, setUsdcToSolRate] = useState<number>(0); // State for conversion rate
  ...
  const fetchProducts = async (query?: GetUsersQuery): Promise<void> => {
    try {
      const rate = await fetchUSDCtoSOLConversionRate(); // Fetch conversion rate
      ...
      setProducts(data.product.map((product: any) => ({
        ...product,
        priceSol: (product.price * rate).toFixed(2) // Calculate price in SOL
      })));
      setTotalProduct(data.total);
    } catch (error) {
      console.log(error);
  ...
  useEffect(() => {
    fetchProducts(router.query);
  }, [router.isReady, router.query, usdcToSolRate]); // Add usdcToSolRate as dependency
  ...
  const sendSolPayment = async (amountSol: number, recipientPublicKey: string) => {
    try {
      const connection = new Connection("https://api.mainnet-beta.solana.com", "confirmed");
      const payer = window.solana.publicKey; // Assume user has connected their wallet
